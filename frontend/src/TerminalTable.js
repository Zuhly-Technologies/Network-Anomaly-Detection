...
const TerminalTable = () => {
  ...
  const columns = ["TERMINAL_ID", "x_terminal_id", "y_terminal_id"];
  ...
  const fetchData = async (limit, page) => {
    const res = await fetch(
      `${config.apiUrl}/terminal_table/limit/${limit}/page/${page}`
    );
  ...
  const handleNext = async () => {
    ...
    const res = await fetch(
      `${config.apiUrl}/terminal_table/limit/${limit}/page/${nextPage}`
    );
    const json = await res.json();
    setRows(JSON.parse(json));
    setPage(nextPage);
  ...
  const handlePrevious = async () => {
    ...
    if (previousPage > 0) {
      const res = await fetch(
        `${config.apiUrl}/terminal_table/limit/${limit}/page/${previousPage}`
      );
      const json = await res.json();
      setRows(JSON.parse(json));
  ...
  const handleFirstPage = async () => {
    const res = await fetch(
      `${config.apiUrl}/terminal_table/limit/${limit}/page/1`
    );
    const json = await res.json();
    setRows(JSON.parse(json));
    setPage(1);
  ...
  const handleLastPage = async () => {
    ...
    const resData = await fetch(
      `${config.apiUrl}/terminal_table/limit/${limit}/page/${lastPage}`
    );
  ...
  const handleRegenFormSubmit = (event) => {
    ...
    const newData = {
      terminals: regenFormData.terminals,
    };

    setIsRegen(true);

    fetch(`${config.apiUrl}/regen_terminal_table`, {
      method: "POST",
      headers: {
  ...
  const fetchTableEntries = () => {
    fetch(`${config.apiUrl}/table_entries`)
      ...
      .then((data) => {
        console.log(data);
        setTableEntries({
          count_terminal_profile: data.count_terminal_profile,
        });
      })
  ...
  useEffect(() => {
    document.title = "Terminal Profiles";
  }, []);
    ...
    <div className="app-container">
      ...
      <nav className="navbar">
        <ul className="nav-list">
          ...
          <li className="nav-item">Merchant Profiles</li>
      ...
      <div className="form-container">
        <h2>Regenerate Terminal profiles</h2>
        <form onSubmit={handleRegenFormSubmit}>
          <div className="form-row-stream-table">
            <label htmlFor="head">Number of Terminals</label>
            <input
              type="number"
              name="terminals"
              required
              ...
              value={regenFormData.terminals}
              placeholder={`current count: ${tableEntries.count_terminal_profile}`}
            />
          </div>
          ...
          <tbody>
            {rows.map((d) => (
              <tr>
                <td
                  className="link"
                  onClick={() => handleOpenModal(d[columns[0]], 3)}
                >
                  {d[columns[0]]}
                </td>
                <td>{d[columns[1]]}</td>
