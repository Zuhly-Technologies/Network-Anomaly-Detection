...
#                 # Store predictions
#                 model_predictions.append(pd.DataFrame({'Actual': y_test, 'Predicted': predict, 'Probability': predict_proba}))
...
#                 print('%-17s %-17s  %-15s %-15s %-15s %-15s %-15s %-20s %-25s' % (j[0:-4], ii, str(round(avg_accuracy, 2)), str(round(avg_precision, 2)), 
#                     str(round(avg_recall, 2)), str(round(avg_f1, 2)), str(round(avg_time, 4)), str(round(avg_opt_threshold, 3)), str(round(avg_f1_opt_threshold, 3)))) # the result of the ten repetitions is printed on the screen
...
#     # combined_predictions.to_csv("./results/all_predictions.csv", index=False)
#     print("Total operation time: = ", time.time()- seconds ,"seconds")
    ...
    for j in csv_files:
        ...
        for ii in ml_list:
            for i in range(repetition):
                ...
                X_test[f'Predicted_{ii}'] = predict
                X_test[f'Probability_{ii}'] = predict_proba

                predictions_data.append(X_test.copy())

    combined_predictions = pd.concat(predictions_data, ignore_index=True)
    print("inserting into db")
    combined_predictions.to_sql("predictions_table", 
                                engine, 
                                if_exists='replace', 
                                index=False,
                                chunksize=100000
